...
function setdb(data,page){
	...
	data2["pickupstart"]=JSON.stringify(pickupstart);
	data2["pickuplen"]=JSON.stringify(pickuplen);
...
function getCookiesAux(){
	...
	pickupstart=JSON.parse($.cookie("pickupstart"));
...
function itin_addplace(element,prevevent,nextevent,init){
        ...
        var eventdiv=""+
        "<div class='eventdiv' id='"+evtid+"' data-id='"+evtid+"'>"+
        "<div class='travel'></div>"+
        "<div class='eventdata' style='min-height:"+evtheight+"' data-id='"+evtid+"'>"+
        img+
